...
test('POST /cardLists 201', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ name: 'test', quantity: 'test', cardListType: 'test', cardListTypeId: 'test', createDate: 'test', createdBy: 'test', modifiedDate: 'test', modifiedBy: 'test', isDeleted: 'test', deleteComment: 'test'  })
  expect(status).toBe(201)
  expect(typeof body).toEqual('object')
  expect(body.name).toEqual('test')
  expect(body.quantity).toEqual('test')
  expect(body.cardListType).toEqual('test')
  expect(body.cardListTypeId).toEqual('test')
...
test('GET /cardLists/:id 200', async () => {
  const { status, body } = await request(app())
    .get(`${apiRoot}/${cardList.id}`)
  expect(status).toBe(200)
  expect(typeof body).toEqual('object')
  expect(body.id).toEqual(cardList.id)
})
