...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Libby Crm Rest Api account.<br>
        ...
        &mdash; Libby Crm Rest Api Team
      `
      return sendMail({ toEmail: email, subject: 'Libby Crm Rest Api - Password Reset', content })
    })
